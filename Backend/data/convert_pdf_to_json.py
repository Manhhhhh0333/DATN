...
def parse_bo_thu_from_pdf():
    ...
    all_text = re.sub(r'Trung tâm Hoa Ngữ Tầm Nhìn Việt', '', all_text)
    all_text = re.sub(r'214 BỘ THỦ TIẾNG TRUNG', '', all_text)
    all_text = re.sub(r'STT Mặt chữ Phiên âm Tên Bộ Ý Nghĩa', '', all_text)
    all_text = re.sub(r'Tamnhinviet_vvs', '', all_text)
    all_text = re.sub(r'\d+\.\s+Bộ thủ \d+ nét.*?\)', '', all_text)
    ...
    for number in range(1, 215):
        pattern = rf'\b{number}\s+([^\s(]+)\s*(?:\(([^)]+)\))?\s+([A-Za-zāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜü]+)\s+([A-Z\s,()]+?)\s+([^0-9]+?)(?=\s+\d+\s+|$)'
        ...
        variant_str = match.group(2) if match.group(2) else None
        pinyin = match.group(3).strip()
        vietnamese_name = match.group(4).strip()
