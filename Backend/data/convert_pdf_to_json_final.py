...
def parse_bo_thu_from_pdf():
    ...
    entries = {}
    i = 0
    prev_vietnamese_part = None
    
    while i < len(all_lines):
        line = all_lines[i]
        
        if re.match(r'^[A-Z\s,()]+$', line) and '(' in line and ')' not in line:
            prev_vietnamese_part = line
            i += 1
            continue
            ...
            if 1 <= number <= 214:
                ...
                if char_match:
                    ...
                    if pinyin_match:
                        ...
                        if prev_vietnamese_part:
                            vietnamese_name = prev_vietnamese_part
                            prev_vietnamese_part = None
                            
                            if i + 1 < len(all_lines):
                                next_line = all_lines[i + 1]
                                if re.match(r'^[A-Z\s,()]+\)$', next_line.strip()):
                                    vietnamese_name += " " + next_line.strip()
                                    i += 1
                        else:
                            vietnamese_match = re.match(r'^([A-Z\s,()]+)', rest)
                            if vietnamese_match:
                                vietnamese_name = vietnamese_match.group(1).strip()
                                meaning = rest[len(vietnamese_name):].strip()
                        
                        if not vietnamese_name and i + 1 < len(all_lines):
                            next_line = all_lines[i + 1]
                            if re.match(r'^[A-Z\s,()]+', next_line.strip()):
                                vietnamese_name = next_line.strip()
                                i += 1
                                ...
                                if i + 1 < len(all_lines):
                                    ...
                                    if re.match(r'^[A-Z\s,()]+\)$', next_next_line.strip()):
                                        vietnamese_name += " " + next_next_line.strip()
                                        i += 1
